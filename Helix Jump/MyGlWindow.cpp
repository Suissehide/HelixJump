...
static double DEFAULT_VIEW_POINT[3] = {0, 75, 43};
...
//==========================================================================
{
	...
	cylinder = Cylinder(3.0f ,3.0f, 50.0f, 36, 1, true);

	platforme = Cylinder(cylinder.getBaseRadius()*2.5, cylinder.getBaseRadius() * 2.5, cylinder.getBaseRadius()/2, 36, 1, true);
	hole = Cylinder(cylinder.getBaseRadius()*2.5, cylinder.getBaseRadius() * 2.5, cylinder.getBaseRadius(), 3, 1, true);
	side = Cylinder(cylinder.getBaseRadius(), cylinder.getBaseRadius(), cylinder.getBaseRadius()/2, 3, 1, true);
...
//Draw axises
  ...
  glTranslatef(0, 0, -cylinder.getHeight() / 2);
  cylinder.draw();
  glPopMatrix();
  ...
  while (height < cylinder.getHeight()) {
	  glRotatef(55.0f, 0, 0, 1.0f);
...
//==========================================================================
{
	std::cout << m_viewer->getViewPoint().x << std::endl << m_viewer->getViewPoint().y << std::endl << m_viewer->getViewPoint().z << std::endl << std::endl;
  switch(e) {
  ...
  case FL_DRAG: // if the user drags the mouse
	  {
		  ...
		  if( m_pressedMouseButton == 1 ) {
			  m_viewer->rotate( fractionChangeX, 0);
		  } else if( m_pressedMouseButton == 2 ) {
			  m_viewer->zoom( fractionChangeY );
		  } else if( m_pressedMouseButton == 3 ) {
			  m_viewer->translate( -0, -fractionChangeY, (Fl::event_key(FL_Shift_L) == 0) || (Fl::event_key(FL_Shift_R) == 0));
		  } else {
